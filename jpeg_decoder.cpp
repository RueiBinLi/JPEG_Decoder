...
public:
    ...
    // Optimized Canonical Huffman Decode
            ...
            for (int mx = 0; mx < num_x; mx++) {
                ...
                for (int i = 0; i < sof_info.components.size(); i++) {
                    ...
                    for (int b = 0; b < mcu_data[i].size(); b++) {
                         decode_block(bs, 
                                      dc_tables[comp.dc_table_id], 
                                      ac_tables[comp.ac_table_id], 
                                      reinterpret_cast<int&>(last_dc[dc_idx]),
                                      comp.quant_table_id,
                                      mcu_data[i][b].data());
                    }
                }
                ...
                for (int y = 0; y < mcu_h; y++) {
                    for (int x = 0; x < mcu_w; x++) {
                        ...
                        auto get_val = [&](int c_idx, float& val) {
                            ...
                            val = mcu_data[c_idx][blk_idx][(sy % 8) * 8 + (sx % 8)];
                        };

                        if (sof_info.components.size() >= 1) get_val(0, y_val);
                        if (sof_info.components.size() >= 2) get_val(1, cb_val);
                        if (sof_info.components.size() >= 3) get_val(2, cr_val);
